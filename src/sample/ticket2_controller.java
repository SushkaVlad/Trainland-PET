...
public class ticket2_controller {
    ...
    public void zatichka(){
        ...
        buy_button.setOnAction(e ->{
            ...
            String tick_documentText = document_ticket_field.getText().trim();//считываем все текстовые поля, убрав ненужные пробелы
            Boolean ch = true;
            checkemail(tick_emailText);
            checksurname(tick_surnameText);
            checkname(tick_nameText);
            checkfathername(tick_fathernameText);
            checkdocument(tick_documentText);//последовательно проверяем все заполненные поля(в случае ошибки в соответствующем label высветиться причина)
            ...
            {ch=false;}
            if (ch)//только в случае корректного заполнения всех полей произойдет покупка билета
            {
                ...
                //добавили пассажира в заранее подготовленный hashmap, чтобы подсадить его в ближайший поезд, следующий по его марщруту
                ...
                });//на странице авторизации отдельным потоком появляется исчезающая надпись "Покупка билета прошла успешно"
                thread.start();
                primaryStage.getScene().setRoot(authroot);
                primaryStage.setTitle("Авторизация");
                primaryStage.show();
            }
        });//нажатие на кнопку "Покупка билета"
    ...
    }//корректность заполнения поля "Email"
    ...
    private boolean checkfathername(String fathernameText)
    {
        ...
        if (ch) {return true;} else {return false;}
    }//корректность заполнения поля "Отчество"
    ...
    private boolean checkdocument(String documentText)
    {
        ...
        if (ch) {return true;} else {return false;}
    }//корректность заполнения поля "Документ, подтверждающий личность"
