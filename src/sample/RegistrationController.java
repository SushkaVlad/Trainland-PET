...
public class RegistrationController {
    ...
    void setStage1(Stage primarystage){
    ...
    void initialize() {
        ...
        {   String emailText = email_registration_field.getText().trim();
            ...
            String captchaText = captcha_registration_field.getText().trim();//считываем все текстовые поля, убрав ненужные пробелы
            for (int i = 0; i<labels.length;i++) {labels[i].setText("");}
            boolean ch = true;
            boolean login_ex = true;

            checkemail(emailText);
            checklogin(loginText);
            try {
                 login_ex = check_existence_login(loginText);
            ...
            checkconfidency();
            checkcaptcha(captchaText, captcha);//последовательно проверяем все заполненные поля(в случае ошибки в соответствующем label высветиться причина)
            if (!checkemail(emailText)||!checklogin(loginText)||!checkpassword(passwordText)||!checkpassword2(password2Text,passwordText)||
                    !checkname(nameText)||!checksurname(surnameText)||!checkphone(phoneText)||!checkconfidency()||!checkcaptcha(captchaText, captcha)||!login_ex)
            {ch=false;}

            if (ch)//только в случае корректного заполнения всех полей вносим пользователя в базу данных
            {
                ...
                });//на странице авторизации отдельным потоком появлется исчезающая надпись "Регистрация прошла успешно"
                thread.start();
        ...
        back.setOnMouseClicked(e ->{
            ...
            primaryStage.setTitle("Авторизация");
            primaryStage.show();
        });//возвращение к странице авторизации
    ...
    }//корректность заполнения поля "E-mail"
    ...
    private boolean checklogin(String loginText)
    {
        ...
        if (ch) {return true;} else {return false;}
    }//корректность заполнения поля "Логин"
    ...
    private boolean checkpassword(String passwordText)
    {
        ...
        if (ch) {return true;} else {return false;}
    }//корректность заполнения поля "Пароль"
    ...
    private boolean checkpassword2(String password2Text, String passwordText)
    {
        ...
        if (ch) {return true;} else {return false;}
    }//корректность заполнения поля "Подтверждение пароля"
    ...
    private boolean checkname(String nameText)
    {
        ...
        if (ch) {return true;} else {return false;}
    }//корректность заполнения поля "Имя"
    ...
    private boolean checksurname(String surnameText)
    {
        ...
        if (ch) {return true;} else {return false;}
    }//корректность заполнения поля "Фамилия"
    ...
    private boolean checkphone(String phoneText)
    {
        ...
        if (ch) {return true;} else {return false;}
    }//корректность заполнения поля "Номер телефона"
    ...
    private boolean checkconfidency()
    {
        ...
        if (ch) {return true;} else {return false;}
    }//дано ли согласие на обработку данных
    ...
    private boolean checkcaptcha(String captchaText, String captcha)
    {
        ...
        if (ch) {return true;} else {return false;}
    }//проверяет соответствие введеного текста с изображение капчи
    ...
    String md5Custom(String st) {
        ...
        return md5Hex;
    }//функция, хэширующая пароль(переводит в строку 30-значной длины)
    ...
    private boolean check_existence_login(String loginText) throws SQLException {
        ...
        return true;
    }//проверяем свободен ли логин(в случае если такой логин уже существует, зарегистрировать пользователя не удастся)
